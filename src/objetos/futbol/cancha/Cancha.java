...
public class Cancha implements Serializable {
	...
	public double PosicionRobot_x;
	public double PosicionRobot_y;
	...
	public double calcular_yR() {
		...
		double m = Diametro_rueda * Math.PI * (UsuarioUsuario.Cancha1.Delta_taco);
		return  PosicionRobot_y+(((Math.sin(Memoria_angulo + Angulo_actual)) * (m)));
		/*
	...
	public double calcular_xR() {
		// operaciones geom̩tricas con coseno
		double m = Diametro_rueda * Math.PI * (UsuarioUsuario.Cancha1.Delta_taco);
		return  PosicionRobot_x+(((Math.cos(Memoria_angulo + Angulo_actual)) * (m)));
		/*
		 * m=DiametroLlantas*Pi*cambio_Tacometro xf= ((cos(angulo))*(m))
		 * +x0(posicoin anterior en x)
		 */
